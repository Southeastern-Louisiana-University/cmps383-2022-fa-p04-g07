...
using FA22.P04.Web.Migrations;
using FluentAssertions;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using InitialMigration = FA22.P04.Web.Migrations.Initial;

namespace FA22.P04.Tests.Web
{
    [TestClass]
    public class DataContextTests
    {
        ...
        public void InitialMigration_NotDeletedOrChanged()
        {
            ...
            attribute?.Id.Should().Be("20220830184634_Initial");
        }
        ...
        [TestMethod]
        public void DataContext_HasProduct()
        {
            using var scope = context.GetServices().CreateScope();
            var dbContext = GetDataContext(scope);
            if (dbContext == null)
            ...
            EnsureSet("Product", dbContext);
        }

        [TestMethod]
        public void DataContext_HasListing()
        {
            using var scope = context.GetServices().CreateScope();
            var dbContext = GetDataContext(scope);
            if (dbContext == null)
            ...
            EnsureSet("Listing", dbContext);
        }

        [TestMethod]
        public void DataContext_HasItem()
        {
            using var scope = context.GetServices().CreateScope();
            var dbContext = GetDataContext(scope);
            if (dbContext == null)
